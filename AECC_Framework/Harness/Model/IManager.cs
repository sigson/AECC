...
public abstract
    ...
    class IManager : ProxyBehaviour
    {
        ...
        #endregion
        protected IECSObjectPathContainer _ConnectPoint = new IECSObjectPathContainer(0, true);//todo//alarm//error//exception//warning - shit
            ...
            if (!isPrefabScript)
            {
                if (!NoSetupChild)
                {
                    if (this is IEntityManager)
                        ///ManagerSpace.InstantiatedProcess(this.gameObject, (IEntityManager)this);///we have the proble
                    if (SetupAction != null)
                        SetupAction(this);
                }
                OnStartManager();
