        ...
        public IDictionary<TKey, TValue> dictionary
        {
            get
            {
                ...
                if (KernelRuntime.DefaultMode == ConcurrencyMode.SingleThread)
                {
                    ...
                    fresh = SimpleDictionary;
                }
                else
                {
                    if (ConcurrentDictionary == null)
                        ConcurrentDictionary = new ConcurrentDictionary<TKey, TValue>();
                    fresh = ConcurrentDictionary;
                }
                var raced = System.Threading.Interlocked.CompareExchange(ref _latched, fresh, null);
                return raced ?? fresh;
            }
        }
