...
{
    /// <summary>
    /// Событийный реестр отложенной десериализации. Объект, чья десериализация не
    /// завершилась из-за ещё не пришедшей ссылки (корневой сущности пути, а её компоненты
    /// приходят в рамках того же AddNewEntity), регистрирует здесь повторную попытку.
    /// Попытки сливаются при приходе сущностей (ECSEntityManager.AddNewEntityReaction).
    ///
    ...
    /// потребителе, здесь — только момент повторной попытки.
    ///
    /// ─── Гейт пустого реестра + коалесинг слива ───
    /// Точка входа события — <see cref="RequestDrain"/>: ноль аллокаций при пустом
    /// реестре, не более ОДНОГО запланированного дрейнера одновременно (CAS по
    /// _drainActive), пропущенные во время слива события доигрываются циклом дрейнера
    /// (эпоха _requests).
    ///
