...
namespace AECC.Core.BuiltInTypes.Types.AtomicType
{
    ...
    public class IECSObjectPathContainer : BaseCustomType
    {
        ...
        public IECSObjectPathContainer(long worldId, bool enableClientBehaviour = false, bool updateCache = false)
        {
            this.ECSWorldOwnerId = worldId;
            AlwaysUpdateCache = updateCache;
            if (enableClientBehaviour)
            {
                if (this.ECSWorldOwner != null && this.ECSWorldOwner.Profile.AlwaysUpdatePathCache)
                {
                ...
                if (cacheECSObject == null && pathToECSObject.Count > 0)
                    {
                        ...
                        foreach (var pathelement in pathToECSObject)
                        {
                            ...
                            if (objectType == "ent")
                            {
                                this.ECSWorldOwner.entityManager.TryGetEntitySyncronized(instanceId, out var entt);
                                currentObject = entt;
                            }
