...
7. **Опционально: убрать глобальный лок сумки с горячего пути (микро-латентность).** Сейчас каждый
   доступ к компоненту берёт read-лок уровня сумки (для взаимоисключения с `EnterLockdown`/
   `ClearSnapshot`). Lockdown по одной сущности — редкость. Если важна однопоточная латентность лока
   (в многопоточном throughput она уже не узкое место), глобальный read-лок можно заменить на
   счётчик in-flight операций (`Interlocked.Increment`/`Decrement` + проверка флага `_lockdown`),
   а `EnterLockdown` — на «поднять флаг и дождаться in-flight==0». Это снимает один из двух локов на
   операцию. Не включено: меняет семантику lockdown, требует прогона тестов на реальном дереве.
