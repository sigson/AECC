    ...
    public class TcpSession : IDisposable
    {
        ...
        internal void Connect(Socket socket)
        {
            ...
            if (Server.OptionKeepAlive)
            {
                ...
                // SIO_KEEPALIVE_VALS = 0x98000004 (Windows)
                // Структура: [onoff (4 bytes)] [keepalivetime ms (4 bytes)] [keepaliveinterval ms (4 bytes)]
                if (Server.OptionTcpKeepAliveTime >= 0 || Server.OptionTcpKeepAliveInterval >= 0)
                {
                    byte[] keepAliveValues = new byte[12];
                    BitConverter.GetBytes((uint)1).CopyTo(keepAliveValues, 0); // onoff = 1

                    uint keepAliveTimeMs = Server.OptionTcpKeepAliveTime >= 0
                        ? (uint)(Server.OptionTcpKeepAliveTime * 1000)
                        : 7200000; // default 2 часа

                    uint keepAliveIntervalMs = Server.OptionTcpKeepAliveInterval >= 0
                        ? (uint)(Server.OptionTcpKeepAliveInterval * 1000)
                        : 1000; // default 1 сек

                    BitConverter.GetBytes(keepAliveTimeMs).CopyTo(keepAliveValues, 4);
                    BitConverter.GetBytes(keepAliveIntervalMs).CopyTo(keepAliveValues, 8);
        ...
        public virtual long Send(ReadOnlySpan<byte> buffer)
        {
            ...
            // Конвертируем Span в byte[]
            byte[] data = buffer.ToArray();

            long sent = Socket.Send(data, 0, data.Length, SocketFlags.None, out SocketError ec);
