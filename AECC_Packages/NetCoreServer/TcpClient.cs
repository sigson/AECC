    ...
    public class TcpClient : IDisposable
    {
        ...
        public virtual bool Connect()
        {
            ...
            if (OptionKeepAlive)
            {
                ...
                if (OptionTcpKeepAliveTime >= 0 || OptionTcpKeepAliveInterval >= 0)
                {
                    byte[] keepAliveValues = new byte[12];
                    BitConverter.GetBytes((uint)1).CopyTo(keepAliveValues, 0); // onoff = 1

                    uint keepAliveTimeMs = OptionTcpKeepAliveTime >= 0
                        ? (uint)(OptionTcpKeepAliveTime * 1000)
                        : 7200000; // default 2 часа

                    uint keepAliveIntervalMs = OptionTcpKeepAliveInterval >= 0
                        ? (uint)(OptionTcpKeepAliveInterval * 1000)
                        : 1000; // default 1 сек
        ...
        public virtual long Send(ReadOnlySpan<byte> buffer)
        {
            ...
            long sent = Socket.Send(arr, 0, arr.Length, SocketFlags.None, out SocketError ec);
            if (sent > 0)
            {
                // Update statistic
                BytesSent += sent;
